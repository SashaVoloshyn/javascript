...
for (let i = 0; i < simpsons.length; i++) {
     document.write(`<div
                        <h2>${simpsons[i].name}, ${simpsons[i].surname}, </br>Age is - ${simpsons[i].age}</h2>
                         <p>${simpsons[i].info} </p>
                         <img  src="${simpsons[i].photo}" alt="${simpsons[i].name}">
                     </div>`);
...
for (i = 0; i < products.length; i++) {
    document.write(`<div class="product-card" style="
margin: 0 auto;background: linear-gradient(to right, #8affd5, #b03fb4); color: black;
width: 100%; display: flex;flex-direction: column; justify-content: center;align-items: center;">
                            <h3 class="product-title">${products[i].title} ${products[i].price} </h3>
                            <img style="width: 200px" src="${products[i].image}" alt="${products[i].title}">
